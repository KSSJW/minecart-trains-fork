...
public class MinecartTrains implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> {
            ...
            if (stored != null && !cart.getUuid().equals(stored)) {
                if (server.getEntity(stored) instanceof AbstractMinecartEntity parentEntity) {
                    // ✅ 强制转为 IChainable 接口类型
                    IChainable parent = (IChainable) (Object) parentEntity;
                    IChainable child = (IChainable) (Object) cart;

                    Set<IChainable> train = new HashSet<>();
                    train.add(parent);

                    AbstractMinecartEntity nextParentEntity;
                    while ((nextParentEntity = ((IChainable) (Object) parent).getChainedParent()) != null
                            && !train.contains((IChainable) (Object) nextParentEntity)) {
                        train.add((IChainable) (Object) nextParentEntity);
                        parent = (IChainable) (Object) nextParentEntity;
                    }

                    // 判断是否循环或已连接
                    if (train.contains(child) || ((IChainable) (Object) parentEntity).getChainedChild() != null) {
                        player.sendMessage(new TranslatableText(MOD_ID + ".invalid_chaining").formatted(Formatting.RED), true);
                    } else {
                        // 若已有父节点则解绑
                        if (((IChainable) (Object) cart).getChainedParent() != null) {
                            IChainable.unsetChainedParentChild(
                                    (IChainable) (Object) cart,
                                    (IChainable) (Object) ((IChainable) (Object) cart).getChainedParent()
                            );
                        }
                        // ✅ 建立新的父子连接
                        IChainable.setChainedParentChild((IChainable) (Object) parentEntity, (IChainable) (Object) cart);
                    }
                } else {
                ...
                world.playSound(null, cart.getX(), cart.getY(), cart.getZ(),
                        SoundEvents.BLOCK_CHAIN_PLACE, SoundCategory.NEUTRAL, 1f, 1.1f);
                ...
                world.playSound(null, cart.getX(), cart.getY(), cart.getZ(),
                        SoundEvents.BLOCK_CHAIN_HIT, SoundCategory.NEUTRAL, 1f, 1.1f);
            }
    ...
    private void loadOrCreateConfig() {
        Gson gson = new GsonBuilder()
                .setPrettyPrinting()
                .registerTypeAdapter(MCTConfig.class, (com.google.gson.JsonDeserializer<MCTConfig>) (json, type, ctx) -> {
                    var obj = json.getAsJsonObject();
                    boolean enable = obj.has("enableCartChaining") && obj.get("enableCartChaining").getAsBoolean();
                    return new MCTConfig(enable);
                })
                .create();
